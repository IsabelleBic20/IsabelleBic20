...
def from_github(user: str, token: str | None, limit: int, exclude: set[str], curve: float):
    ...
    while True:
        try:
            batch = _api(f"https://api.github.com/users/{user}/repos?per_page=100&page={page}&type=owner", token)
    ...
    for r in repos:
        ...
        for name, bytes_ in langs.items():
            if name.lower() in exclude:
                continue
            totals[name] = totals.get(name, 0) + bytes_

    top = sorted(totals.items(), key=lambda kv: kv[1], reverse=True)[:limit]
    if not top:
        sys.exit(f"no qualifying language data for user {user}")
    max_b = max(b for _, b in top)
    axes = [(name, round(100.0 * (b / max_b) ** curve, 1)) for name, b in top]
...
def render(title, axes, theme: str, size: int, rings: int, show_values: bool, animate: bool) -> str:
    ...
    parts.append(
        f'<polygon points="{d}" fill="{c["fill"]}" fill-opacity="0.20" '
        f'stroke="{c["stroke"]}" stroke-width="2.2" stroke-linejoin="round"/>'
    )
    for x, y in shape:
        parts.append(
