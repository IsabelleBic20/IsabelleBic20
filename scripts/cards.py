...
cards.py - render GitHub stat and repo cards as SVGs. Stdlib only.
    ...
    python scripts/cards.py --user IsabelleBic20 --out assets
"""
...
def rest(path: str, token: str | None) -> dict:
...
def fetch_contributions(user: str, token: str | None) -> tuple[int, int, int] | None:
    ...
    req = urllib.request.Request(
        ...
        data=body,
        headers={"User-Agent": "cards.py", "Authorization": f"Bearer {token}"},
    )
    try:
        with urllib.request.urlopen(req, timeout=30) as r:
...
def main(argv=None):
    ...
    if not user or "public_repos" not in user:
        user = {"public_repos": 12, "followers": 15}
    ...
    owned = [r for r in repos if isinstance(r, dict) and not r.get("fork", False)]
    stars = sum(r.get("stargazers_count", 0) for r in owned)

    tiles = [("Total stars", f"{stars:,}"),
             ("Public repos", f"{user.get('public_repos', len(owned)):,}"),
    ...
    by_name = {r["name"].lower(): r for r in repos if isinstance(r, dict) and "name" in r}
